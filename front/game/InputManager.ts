...
export class InputManager {
  ...
  private cameraFollowSystem: OrbitCameraFollowSystem | null = null
  private keyboardListenersAttached = false

  constructor(
    private webSocketManager: WebSocketManager,
    cameraFollowSystem: OrbitCameraFollowSystem | null
  ) {
    // FIXED: Handle optional cameraFollowSystem
  ...
  // FIXED: Lazy initialization for keyboard listeners
  private attachKeyboardListeners() {
    if (this.keyboardListenersAttached) return
  ...
  // FIXED: Public method to attach listeners if delayed
  public ensureInitialized(cameraFollowSystem: OrbitCameraFollowSystem) {
    if (!this.cameraFollowSystem) {
      this.cameraFollowSystem = cameraFollowSystem
      this.attachKeyboardListeners()
    }
  }
  ...
  // FIXED: Added safety check for cameraFollowSystem
  public handleJoystickMove(joystick: IJoystickUpdateEvent) {
    if (!this.cameraFollowSystem) return
  ...
  update(entities: Entity[], dt: number) {
    if (this.pcUser && this.cameraFollowSystem) {
      this.inputState.y = this.cameraFollowSystem.y
    }
    this.proximityPromptSystem.update(entities, dt)
  }

  // FIXED: Direct mobile jump button handlers
  public setJump(value: boolean) {
    this.inputState.s = value
  }

  public setInteract(value: boolean) {
    this.inputState.i = value
  }

  public setMoveUp(value: boolean) {
    this.inputState.u = value
  }

  public setMoveDown(value: boolean) {
    this.inputState.d = value
  }

  public setMoveLeft(value: boolean) {
    this.inputState.l = value
  }

  public setMoveRight(value: boolean) {
    this.inputState.r = value
  }
