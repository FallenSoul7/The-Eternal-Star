...
export class Game {
  ...
  private constructor(gameContainerRef: MutableRefObject<any>, slug: string) {
    ...
    // FIXED: Initialize InputManager with optional cameraFollowSystem (may not be ready yet)
    this.inputManager = new InputManager(this.websocketManager, this.renderer.camera?.controlSystem || null)
    this.hud = new Hud()
  }
  ...
  async start() {
    ...
    // FIXED: Ensure InputManager is fully initialized with camera system
    if (this.renderer.camera?.controlSystem) {
      this.inputManager.ensureInitialized(this.renderer.camera.controlSystem)
    }
